...
import java.util.*;
public class Bc1t extends SaltoCondicional implements PlanifDinamica{

    Registro r[]; 
    String etiqueta;
    private int posIns;
    public Bc1t(String param[], int p)
    {
   ...
   public void inicia()
   {
    ...
    public int ejecutar()
    {
        if(Status.getRegistro().getPalabra().getDec()==1)
        {
    ...
    public int etapa3()
    {
        ...
        if(Status.getRegistro().getPalabra().getDec()==1)
        {
            ...
            Pc.getRegistro().modificar(i);
            boolean_pc=true;
            imagen_etapa.append(direccion_multi).append("bc1t");        
        }
        else
            imagen_etapa.append(direccion_multi).append("bc1t_no");        
        return -1;   
    ...
    {        
        ...
        String codigo;
        codigo=setOp(17)+setR(8)+setR(1)+setIn(posSalto);       
