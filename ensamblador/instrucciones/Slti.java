...
public class Slti extends Inmediatas {
    
    Registro r[];
    int inmediato;
    
    public Slti(String param[]) 
    {
        int posi=-1;
	...
	if (inmediato>32767){
	    if(r[1].getPalabra().getDec()< (inmediato-65536))
		r[0].setPalabra(new Palabra(1));
	    else
		r[0].setPalabra(new Palabra(0)); 
	}
	else {
	    if(r[1].getPalabra().getDec()< inmediato)
		r[0].setPalabra(new Palabra(1));
	    else
		r[0].setPalabra(new Palabra(0)); 
	}                
...
/*
    ...
    {
        String codigo;
        codigo=setOp(10)+setR(r[1].numReg())+setR(r[0].numReg())+setIn(inmediato);       
